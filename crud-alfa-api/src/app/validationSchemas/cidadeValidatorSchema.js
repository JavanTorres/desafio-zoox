...
const insertBodyValidatorCidade = Joi.object().keys({
  nome: defaultString.required(),
  estadoId: defaultString.hex().min(24).required(),
});

const updateBodyValidatorCidade = Joi.object().keys({
  nome: defaultString.required(),
  estadoId: defaultString.hex().min(24).required(),
});

const queryValidatorCidade = Joi.object().keys({
  nome: defaultString,
});

const paramValidatorCidade = Joi.object().keys({
  _id: defaultString.hex().min(24).required(),
});

module.exports = {
  options,
  insertBodyValidatorCidade,
  queryValidatorCidade,
  paramValidatorCidade,
  updateBodyValidatorCidade,
};
